...
public class ExtensionDemo extends Application {
  ...
  private void render(String markdown, VBox outputBox) {
    ...
    MarkdownParser parser = MarkdownParser.builder().extensions(greetingExtension).build();

    JavaFxRenderer renderer = JavaFxRenderer.builder().extensions(greetingExtension).build();
  ...
  static class GreetingExtension
      implements ParserExtension, HtmlRendererExtension, JavaFxRendererExtension {
    @Override
  ...
  static class GreetingJavaFxRenderer implements JavaFxNodeRenderer {
    ...
    public void render(Node node) {
      ...
      label.setStyle(
          "-fx-font-size: 16px; -fx-padding: 10px; -fx-background-color: #e3f2fd; -fx-border-color:"
              + " #2196f3; -fx-border-width: 0 0 0 4px;");
