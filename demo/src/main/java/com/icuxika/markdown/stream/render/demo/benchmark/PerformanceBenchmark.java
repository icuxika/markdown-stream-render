  ...
  private void runVboxTest() throws InterruptedException {
    ...
    Platform.runLater(
        () -> {
          ...
          VBox root = new VBox();
          JavaFxRenderer renderer =
              new JavaFxRenderer(); // Use one renderer for config, but render individual nodes?
          ...
          for (int i = 0; i < ITEM_COUNT; i++) {
            ...
            p.appendChild(
                new Text(
                    "This is paragraph #" + i + " in a VBox. It consumes memory and layout time."));
    ...
    Platform.runLater(
        () -> {
          ...
          for (int i = 0; i < ITEM_COUNT; i++) {
            Paragraph p = new Paragraph();
            p.appendChild(
                new Text("This is paragraph #" + i + " in a ListView. It is virtualized."));
            items.add(p);
          }
