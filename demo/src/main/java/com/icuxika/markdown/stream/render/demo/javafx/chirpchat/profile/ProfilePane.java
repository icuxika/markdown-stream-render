...
public class ProfilePane extends VBox {
  ...
  private VBox createProfileHeader() {
    ...
    if (user.isVerified()) {
      ...
      verified.setStyle(
          "-fx-background-color: #1DA1F2; -fx-background-radius: 10; -fx-text-fill: white;"
              + " -fx-font-size: 11px; -fx-padding: 2px 5px;");
    ...
    Label joinDate =
        new Label(
            "📅 Joined " + (user.getJoinDate() != null ? user.getJoinDate() : "January 2024"));
  ...
  private VBox createPostsSection() {
    ...
    Label content =
        new Label(
            "Welcome to my profile! I'm excited to connect with you all. Let's build something"
                + " amazing together! 🚀");
    ...
    Label sampleText =
        new Label("Working on some exciting new features! Stay tuned for updates. 💻✨");
