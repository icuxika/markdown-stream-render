      ...
      try (OutputStream os = t.getResponseBody()) {
        ...
        String bodyStart =
            "<div class='app-container'>"
                ...
                + "<div id='content' class='markdown-root' data-theme='light'>";
        ...
        StreamMarkdownParser.Builder parserBuilder =
            StreamMarkdownParser.builder().renderer(renderer);
        ...
        String bodyEnd =
            "</div>"
                ...
                + "</div>"
                + // close app-container
                "<script>function applyTheme(next) {  const html = document.documentElement; "
              + " html.setAttribute('data-theme', next); "
              + " document.querySelectorAll('.markdown-root').forEach(el =>"
              + " el.setAttribute('data-theme', next));}function toggleTheme() {  const html ="
              + " document.documentElement;  const current = html.getAttribute('data-theme') ||"
              + " 'light';  const next = current === 'dark' ? 'light' : 'dark'; "
              + " applyTheme(next);}applyTheme(document.documentElement.getAttribute('data-theme')"
              + " || 'light');"
                +
                // Auto-scroll logic
                "const contentDiv = document.getElementById('content');const observer = new"
                + " MutationObserver(() => {  window.scrollTo(0, document.body.scrollHeight);});if"
                + " (contentDiv) observer.observe(contentDiv, { childList: true, subtree: true });"
                + "</script></body></html>";
