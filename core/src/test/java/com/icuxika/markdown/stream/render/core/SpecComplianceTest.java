    ...
    Stream<DynamicTest> testGfmCompliance() throws Exception {
        ...
        return tests.stream()
                ...
                .map(testCase -> DynamicTest.dynamicTest(
                        "Example " + testCase.example + " (" + testCase.section + ")",
                        () -> {
                            ...
                            // If we wanted to test output equality, we would need to depend on 'html' module in 'core' tests, 
                            // which would create circular dependency (html depends on core).
                            // So this test mainly verifies that the PARSER doesn't crash on any GFM spec example.
                            assertNotNull(doc);
                        }
                ));
    }
