...
*   **测试套件位置**：`html/src/test/resources/`
    *   `commonmark-spec-*.json`: 核心 CommonMark 规范测试。
    *   `gfm-spec-*.json`: GitHub Flavored Markdown (GFM) 扩展测试。
...
1.  **`core`**：大脑。包含 AST 定义、解析逻辑和通用渲染器接口。**必须保持框架无关**（无 JavaFX/Swing 依赖）。
2.  **`javafx`**：表现层。使用 JavaFX 组件实现渲染器接口。
3.  **`demo`**：可运行的应用程序，用于展示功能（如聊天 UI、文件查看器）。
