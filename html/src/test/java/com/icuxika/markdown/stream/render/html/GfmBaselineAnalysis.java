...
public class GfmBaselineAnalysis {
  ...
  public void runAnalysis() throws IOException {
    ...
    System.out.println(
        "Pass Rate: " + String.format("%.2f%%", (double) totalPassed / examples.size() * 100));
    System.out.println("--------------------------------------------------");
    System.out.println(
        String.format("%-30s | %-10s | %-10s | %-10s", "Section", "Total", "Passed", "Rate"));
    System.out.println("--------------------------------------------------");
    ...
    stats.entrySet().stream()
        ...
        .forEach(
            entry -> {
              ...
              System.out.println(
                  String.format("%-30s | %-10d | %-10d | %6.2f%%", section, total, pass, rate));
            });
    System.out.println("==================================================");
      ...
      stats.entrySet().stream()
          ...
          .forEach(
              entry -> {
                ...
                double rate = (double) pass / total * 100;
                writer.println(
                    String.format("| %s | %d | %d | %.2f%% |", section, total, pass, rate));
              });
    }
