...
public class CoreJavaFxNodeRenderer implements JavaFxNodeRenderer {
  ...
  private static final java.util.Map<String, javafx.scene.image.Image> IMAGE_CACHE =
  ...
  private void renderListItem(ListItem listItem) {
    ...
    if (listItem.isTask()) {
      ...
      checkBox.setAlignment(
          javafx.geometry.Pos.TOP_LEFT); // Align the box itself to top if it has height?
  ...
  private boolean isSqlKeyword(String text) {
    Set<String> keywords =
        Set.of(
            ...
            "CREATE", "TABLE", "DROP", "ALTER", "INDEX", "AND", "OR", "NOT", "NULL", "TRUE",
            "FALSE", "JOIN", "LEFT", "RIGHT", "INNER", "OUTER", "ON", "GROUP", "BY", "ORDER", "ASC",
            "DESC", "LIMIT", "OFFSET", "COUNT", "SUM", "AVG", "MAX", "MIN", "select", "from",
            "where", "insert", "into", "values", "update", "set", "delete", "create", "table",
            "drop", "alter", "index", "and", "or", "not", "null", "true", "false", "join", "left",
            "right", "inner", "outer", "on", "group", "by", "order", "asc", "desc", "limit",
            "offset", "count", "sum", "avg", "max", "min");
  ...
  private void renderImage(Image image) {
    try {
      ...
      javafx.scene.image.Image img =
          IMAGE_CACHE.computeIfAbsent(url, k -> new javafx.scene.image.Image(k, true));
