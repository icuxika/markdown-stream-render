...
public class JavaFxStreamRenderer implements StreamMarkdownTypingRenderer {
  ...
  private final java.util.concurrent.atomic.AtomicBoolean isPreviewDirty =
  ...
  private void initStyles() {
     ...
     * "/com/icuxika/markdown/stream/render/javafx/css/markdown.css");
     * if (cssUrl != null) { String cssPath = cssUrl.toExternalForm(); if
     * (!root.getStylesheets().contains(cssPath))
     * { root.getStylesheets().add(cssPath); } }
     */
  }

  // Batching queue
  private final java.util.concurrent.ConcurrentLinkedQueue<Runnable> pendingUpdates =
      new java.util.concurrent.ConcurrentLinkedQueue<>();
  private final java.util.concurrent.atomic.AtomicBoolean isUpdateScheduled =
      new java.util.concurrent.atomic.AtomicBoolean(false);
  ...
  private void renderNodeOnFxThread(Node node) {
    // Prevent duplication for container blocks that are already handled in
    // openBlock
    if (node instanceof Document
        ...
        || node instanceof AdmonitionBlock) {
      return;
    }
  ...
  private void openBlockOnFxThread(Node node) {
    ...
    if (newContainer != null) {
      ...
      // Leaf blocks or non-container blocks (Paragraph, CodeBlock, Heading, Table,
      // HtmlBlock)
      // do not receive closeBlock from MarkdownParser (they are not added to
      // openContainers stack),
      // so we must NOT push to containerStack to avoid imbalance (popping the parent
      // prematurely or never
      // popping).
    }
  }
