 ...
 * <p>
 * Instead of building a Scene Graph tree, this renderer populates a flat list
 * of top-level Nodes. This list is intended
...
public class VirtualJavaFxStreamRenderer implements StreamMarkdownRenderer {
	...
	public void openBlock(Node node) {
		Platform.runLater(() -> {
			...
			// If stack is empty (excluding Document which we don't push explicitly here if
			// stream parser doesn't),
	...
	public void renderNode(Node node) {
		Platform.runLater(() -> {
			...
			// If this node is in our list (top-level), it has changed state (children
			// added).
